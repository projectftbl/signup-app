...
const NavigationLink = Radium(({ to, title, useIndexLink = true, onClick }) => {
  const styles = {
    base: {
      color: '#999'
    , textDecoration: 'none'
...
export default Radium(({ items, style }) => {
  ...
  return (
    <ul style={[ styles.base, style ]}>
      {items.map((item, i) => 
        <NavigationItem to={item.to} title={item.title} onClick={item.onClick} useIndexLink={item.indexLink}
