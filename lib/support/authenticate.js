...
export default function(options) {
  ...
  const ensureShouldChangePassword = ({ location, password }, redirect) => {
    ensure(location, password, redirect, passwordPath);
  };
  ...
  var component = function(DecoratedComponent) {
    ...
    const mapStateToProps = (state, props) => {
      return { 
        session: sessionSelector(state, props, false) 
      }; 
    };
  ...
  component.onEnter = (store, nextState, replace) => {
    const session = sessionSelector(store.getState(), null, true)
        , shouldChangePassword = passwordSelector(store.getState(), null, true);
