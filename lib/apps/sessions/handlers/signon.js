...
export class SignOn extends Component {
  ...
  componentDidUpdate(previous) {
    const { session, reset, pushState } = this.props;

    if (session.user && !previous.session.user) {
      reset('signOn');
      pushState(null, session.user.shouldChangePassword ? '/profile/password' : '/timeline');
    }
  }
...
export default connect(state => ({ session: state.session }), { reset, pushState, signOn, signOnToFacebook, signOnToTwitter })(SignOn);
