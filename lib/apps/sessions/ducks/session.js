...
export const GRANT_CLAIM = 'ftbl/sessions/session/GRANT_CLAIM';
...
export default function reducer(state = initialState, action) {
  ...
  case GRANT_CLAIM:
    const user = assign({}, state.user);
    if (user.claims == null) user.claims = [];
    user.claims.push(action.payload);
...
export function grantClaim(entity, right) {
  return { type: GRANT_CLAIM, payload: { entity, right }};
};
