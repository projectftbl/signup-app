...
export default State('account')(({ account, onConnect, onDisconnect, dispatch, $, state }) => {
  ...
  return (
    <Row onMouseOver={events.over} onMouseOut={events.out}>
      ...
      <Cell width='10%' align='right' hide={!state[account.id]}>
        <a onClick={_ => onDisconnect(account)} style={{cursor:'pointer'}}>
