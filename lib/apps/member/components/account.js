...
const Account = ({ account, onDisconnect }) => {
  ...
  return (
    <span>
      <a href={account.link} style={{paddingLeft:0}} target='_blank'>{account.name || account.link}</a>
      <a onClick={() => onDisconnect(account)} style={{cursor:'pointer', paddingLeft:10}}><Close/></a>
    </span>
  );
};
